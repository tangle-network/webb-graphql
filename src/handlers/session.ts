...
export const fetchSessionAuthorizes = async (blockNumber: string) => {
  ...
  const currentAuthoritiesAccounts = await api.query.dkg.accountToAuthority.multi(accounts);
  ...
  const [pendingKeyGenThreshold, currentKeyGenThreshold, nextKeyGenThreshold] = await Promise.all([
  ...
  const [pendingSignatureThreshold, currentSignatureThreshold, nextSignatureThreshold] = await Promise.all([
  ...
  const sessionAuthorities = dkgAuthorities
    .map((dkgAuth): SessionDKGAuthority => {
      return {
        ...
        uptime: getIntPercentage(uptimeStore[dkgAuth.accountId] ?? 0, storeSessionCounter),
      };
    })
    .filter((s) => s.accountId !== undefined);

  const { sessionNumber: sessionId, sessionBlock } = await currentSessionId(blockNumber);
  return {
    sessionId,
...
async function ensureThreshold(sessionId: string, value: ThresholdValue, variant: ThresholdVariant) {
...
async function createOrUpdateSessionProposer(sessionId: string, proposerAccount: string) {
 ...
 *
 * */
export const createOrUpdateSession = async ({ blockId, sessionId, ...input }: SessionInput) => {
 ...
 * */
export async function setSessionKey(sessionId: string, blockId: string, keyId: string) {
  //Ensure the session is not already created
  const session = await ensureSession(sessionId, blockId);
