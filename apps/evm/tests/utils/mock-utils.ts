...
export function newMockEvent(input:Bytes): ethereum.Event {
  return new ethereum.Event(defaultAddress, defaultBigInt,
    defaultBigInt, defaultEventDataLogType, newBlock(), newTransaction(input), [], newTransactionReceipt());
}

export function newMockCall(input:Bytes): ethereum.Call {
  return new ethereum.Call(defaultAddress, defaultAddress, newBlock(), newTransaction(input), [], []);
}
...
function newTransaction(input:Bytes): ethereum.Transaction {
  return new ethereum.Transaction(defaultAddressBytes, defaultBigInt, defaultAddress,
    defaultAddress, defaultBigInt, defaultBigInt, defaultBigInt, input, defaultBigInt);
}
