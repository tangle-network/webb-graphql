...
(async () => {
  ...
  const ALICE_ROLE_SEED = new Uint8Array(
    Buffer.from('cb6df9de1efca7a3998a8ead4e02159d5fa99c3e0d4fd6432667390bb4726854', 'hex')
  );
  // Generated by subkey inspect //Bob --scheme Ecdsa
  const BOB_ROLE_SEED = new Uint8Array(
    Buffer.from('79c3b7fc0b7697b9414cb87adcb37317d1cab32818ae18c0e97ad76395d1fdcf', 'hex')
  );
  ...
  await new Promise((resolve) => {
    creatingProfileTx.signAndSend(ALICE, ({ events = [], status }) => {
      if (status.isInBlock) {
  ...
  await new Promise((resolve) => {
    creatingProfileTx.signAndSend(BOB, ({ events = [], status }) => {
      if (status.isInBlock) {
        console.log('[creatingProfileTx] Included at block hash', status.asInBlock.toHex());
        console.log('[creatingProfileTx] Events:');
        events.forEach(({ event: { data, method, section } }) => {
  ...
  await new Promise((resolve) => {
    submittingJobTx.signAndSend(ALICE, ({ events = [], status }) => {
      if (status.isInBlock) {
  ...
  await new Promise((resolve) => {
    submittingJobResultTx.signAndSend(ALICE, ({ events = [], status }) => {
      if (status.isInBlock) {
