...
import { Insertion, VAnchor } from '../../generated/vanchor/vanchor';
import {
  Encryptions,
  ExternalData,
  PublicInputs,
...
export const handleTransaction = (event: Insertion): void => {
  ...
  if (data !== null) {
    ...
    if (externalData) {
      ...
      // Fee work
      const fungibleTokenWrapperAddress = VAnchor.bind(event.address).token();
      let fee = 0;
      switch (externalData[5].toAddress().toHexString()) {
        case '0x0000000000000000000000000000000000000000':
          // Native asset
          let totalValue = event.transaction.value;
          
          break;
        case fungibleTokenWrapperAddress.toHexString():
          // VAnchor ERC20 token
          break;
        default:
          // Other ERC20 token
          break;
      // Some fee object `.add(fee)` or `.add(fee, tokenAddress)`
    }
